...
package com.ahmad.mldetectkotlin.classification;
...
import static com.ahmad.mldetectkotlin.classification.PoseEmbedding.getPoseEmbedding;
import static com.ahmad.mldetectkotlin.classification.Utils.maxAbs;
import static com.ahmad.mldetectkotlin.classification.Utils.multiply;
import static com.ahmad.mldetectkotlin.classification.Utils.multiplyAll;
import static com.ahmad.mldetectkotlin.classification.Utils.subtract;
import static com.ahmad.mldetectkotlin.classification.Utils.sumAbs;
import static java.lang.Math.max;
import static java.lang.Math.min;

/**
 * Classifies {link Pose} based on given {@link com.ahmad.mldetectkotlin.classification.PoseSample}s.
 *
...
public class PoseClassifier {
  ...
  private final List<com.ahmad.mldetectkotlin.classification.PoseSample> poseSamples;
  private final int maxDistanceTopK;
  private final int meanDistanceTopK;
  private final PointF3D axesWeights;

  public PoseClassifier(List<com.ahmad.mldetectkotlin.classification.PoseSample> poseSamples) {
    this(poseSamples, MAX_DISTANCE_TOP_K, MEAN_DISTANCE_TOP_K, AXES_WEIGHTS);
  }

  public PoseClassifier(List<com.ahmad.mldetectkotlin.classification.PoseSample> poseSamples, int maxDistanceTopK,
   ...
   *
   * <p><Since we calculate confidence by counting {@link com.ahmad.mldetectkotlin.classification.PoseSample}s that survived
  ...
  public com.ahmad.mldetectkotlin.classification.ClassificationResult classify(Pose pose) {
    return classify(extractPoseLandmarks(pose));
  }

  public com.ahmad.mldetectkotlin.classification.ClassificationResult classify(List<PointF3D> landmarks) {
    com.ahmad.mldetectkotlin.classification.ClassificationResult result = new com.ahmad.mldetectkotlin.classification.ClassificationResult();
    ...
    PriorityQueue<Pair<com.ahmad.mldetectkotlin.classification.PoseSample, Float>> maxDistances = new PriorityQueue<>(
        maxDistanceTopK, (o1, o2) -> -Float.compare(o1.second, o2.second));
    // Retrieve top K poseSamples by least distance to remove outliers.
    for (com.ahmad.mldetectkotlin.classification.PoseSample poseSample : poseSamples) {
    ...
    PriorityQueue<Pair<com.ahmad.mldetectkotlin.classification.PoseSample, Float>> meanDistances = new PriorityQueue<>(
    ...
    for (Pair<com.ahmad.mldetectkotlin.classification.PoseSample, Float> sampleDistances : maxDistances) {
      com.ahmad.mldetectkotlin.classification.PoseSample poseSample = sampleDistances.first;
      List<PointF3D> sampleEmbedding = poseSample.getEmbedding();
    ...
    for (Pair<com.ahmad.mldetectkotlin.classification.PoseSample, Float> sampleDistances : meanDistances) {
